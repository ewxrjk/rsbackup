...
DeviceKey::DeviceKey(Render::Context &ctx): Render::Grid(ctx) {
  unsigned row = 0;
  for(auto device_iterator: globalConfig.devices) {
    const auto &device = device_iterator.first;
    device_rows[device] = row;
    auto t = new Render::Text(ctx, device, globalConfig.colorGraphForeground,
                              globalConfig.deviceNameFont);
    cleanup(t);
    add(t, 0, row);
    auto r = new Render::Rectangle(ctx, globalConfig.backupIndicatorKeyWidth,
                                   indicator_height, device_color(row));
...
const Color DeviceKey::device_color(unsigned row) const {
  ...
  */
  return globalConfig.deviceColorStrategy->get(row,
...
void HistoryGraphContent::render_data() {
  ...
  double base = floor((row_height + globalConfig.verticalPadding - 1
                       - (indicator_height * globalConfig.devices.size()))
                      / 2)
                + 1;
  for(auto host_iterator: globalConfig.hosts) {
    Host *host = host_iterator.second;
    ...
    for(auto volume_iterator: host->volumes) {
      ...
      for(auto backup: volume->backups) {
        if(backup->getStatus() == COMPLETE) {
          double x = (Date(backup->time) - earliest)
...
TimeLabels::TimeLabels(Render::Context &ctx, HistoryGraphContent &content):
    Render::Container(ctx), content(content) {}
...
void TimeLabels::set_extent() {
  if(width < 0) {
    ...
    while(d <= content.latest) {
      ...
      next.addMonth();
      double xnext =
          (next - content.earliest) * globalConfig.backupIndicatorWidth;
      auto t = new Render::Text(context, "", globalConfig.colorGraphForeground,
...
HistoryGraph::HistoryGraph(Render::Context &ctx):
    Render::Grid(ctx), host_labels(ctx), volume_labels(ctx), device_key(ctx),
    content(ctx, device_key), time_labels(ctx, content) {
  set_padding(globalConfig.horizontalPadding, globalConfig.verticalPadding);
}
...
void HistoryGraph::set_extent() {
  ...
  double row_height = std::max(
      {host_labels.get_maximum_height(), volume_labels.get_maximum_height(),
       (globalConfig.backupIndicatorHeight * globalConfig.devices.size())});
...
void HistoryGraph::addPart(const std::string &partspec) {
  static std::regex partspec_regex(
      "^([^:]+):([0-9]+),([0-9]+)(?::([LCR])([TCB]))?$");
  std::smatch mr;
  ...
  if(part == "host-labels")
    w = &host_labels;
  else if(part == "volume-labels")
    w = &volume_labels;
  else if(part == "content")
    w = &content;
  else if(part == "time-labels")
    w = &time_labels;
  else if(part == "device-key")
    w = &device_key;
  else
    throw SyntaxError("unrecognized graph component '" + part + "'");
...
void HistoryGraph::adjustConfig() {
  ...
  double maxContentWidth =
      globalConfig.graphTargetWidth - (width - content.width);
  ...
  if(width < globalConfig.graphTargetWidth) {
    ...
    // Pick the biggest
    globalConfig.backupIndicatorWidth =
        std::max(globalConfig.backupIndicatorWidth, maxIndicatorWidth);
    content.changed();
    return;
  }
  if(globalConfig.graphTargetWidth > 0
     && width > globalConfig.graphTargetWidth) {
